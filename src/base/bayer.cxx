...
namespace CVD{namespace ColourSpace{
	...
	template <class T, class R>
		struct bayer_sample_rggb {
			static inline void upper_left(T (*out)[3], const T * row, const T * next){
				...
				out[0][0] = row0;
				out[0][1] = static_cast<T>((row1 + next0)/2);
				out[0][2] = next1;
				out[1][0] = static_cast<T>((row0+row2)/2);
				out[1][1] = row1;
				out[1][2] = next1;
			}
			static inline void upper_row(T (*out)[3], const T * row, const T * next){
				...
				T next1 = R::get(next[1]);
				out[0][0] = row0;
				out[0][1] = static_cast<T>((row_1+ row1 + next0)/3);
				out[0][2] = static_cast<T>((next_1+next1)/2);
				out[1][0] = static_cast<T>((row0+row2)/2);
				out[1][1] = row1;
				out[1][2] = next1;
			}
			...
			static inline void odd_left(T (*out)[3], const T * previous, const T * row, const T * next){
				...
				out[0][0] = static_cast<T>((prev0 + next0)/2);
				out[0][1] = row0;
				out[0][2] = row1;
				out[1][0] = static_cast<T>((prev0 + next0 + prev2 + next2)/4);
				out[1][1] = static_cast<T>((row0+row2+prev1+next1)/4);
				out[1][2] = row1;
			}
			static inline void odd_row(T (*out)[3], const T * previous, const T * row, const T * next){
				...
				out[0][0] = static_cast<T>((prev0 + next0)/2);
				out[0][1] = row0;
				out[0][2] = static_cast<T>((row_1+row1)/2);
				out[1][0] = static_cast<T>((prev0 + next0 + prev2 + next2)/4);
				out[1][1] = static_cast<T>((row0+row2+prev1+next1)/4);
				out[1][2] = row1;
			}
			static inline void odd_right(T (*out)[3], const T * previous, const T * row, const T * next){
				...
				out[0][1] = row0;
				out[0][2] = static_cast<T>((row_1+row1)/2);
				out[1][0] = static_cast<T>((prev0 + next0)/2);
				out[1][1] = static_cast<T>((row0+prev1+next1)/3);
				out[1][2] = row1;
			}
			...
			static inline void even_left(T (*out)[3], const T * previous, const T * row, const T * next){
				...
				T next1 = R::get(next[1]);
				out[0][0] = row0;
				out[0][1] = static_cast<T>((row1+prev0+next0)/3);
				out[0][2] = static_cast<T>((prev1+next1)/2);
				out[1][0] = static_cast<T>((row0+row2)/2);
				out[1][1] = row1;
				out[1][2] = out[0][2];
			}
			static inline void even_row(T (*out)[3], const T * previous, const T * row, const T * next){
				...
				T next1 = R::get(next[1]);
				out[0][0] = row0;
				out[0][1] = static_cast<T>((row_1+row1+prev0+next0)/4);
				out[0][2] = static_cast<T>((prev_1+next_1+prev1+next1)/4);
				out[1][0] = static_cast<T>((row0+row2)/2);
				out[1][1] = row1;
				out[1][2] = static_cast<T>((prev1 + next1)/2);
			}
			...
			static inline void lower_left(T (*out)[3], const T * previous, const T * row){
				...
				out[0][1] = row0;
				out[0][2] = row1;
				out[1][0] = static_cast<T>((prev0+prev2)/2);
				out[1][1] = static_cast<T>((row0+row2+prev1)/3);
				out[1][2] = row1;
			}
			...
			static inline void lower_right(T (*out)[3], const T * previous, const T * row){
				...
				out[0][2] = static_cast<T>((row_1+row1)/2);
				out[1][0] = prev0;
				out[1][1] = static_cast<T>((row0+prev1)/2);
				out[1][2] = row1;
			}
		};
	...
	template <class T, class R>
		struct bayer_sample_bggr {
			static inline void upper_left(T (*out)[3], const T * row, const T * next){
				...
				out[0][1] = static_cast<T>((row1 + next0)/2);
				out[0][0] = next1;
				out[1][2] = static_cast<T>((row0+row2)/2);
				out[1][1] = row1;
				out[1][0] = next1;
			}
			static inline void upper_row(T (*out)[3], const T * row, const T * next){
				...
				out[0][2] = row0;
				out[0][1] = static_cast<T>((row_1+ row1 + next0)/3);
				out[0][0] = static_cast<T>((next_1+next1)/2);
				out[1][2] = static_cast<T>((row0+row2)/2);
				out[1][1] = row1;
				out[1][0] = next1;
			}
			...
			static inline void odd_left(T (*out)[3], const T * previous, const T * row, const T * next){
				...
				T next1 = R::get(next[1]);
				T next2 = R::get(next[2]);
				out[0][2] = static_cast<T>((prev0 + next0)/2);
				out[0][1] = row0;
				out[0][0] = row1;
				out[1][2] = static_cast<T>((prev0 + next0 + prev2 + next2)/4);
				out[1][1] = static_cast<T>((row0+row2+prev1+next1)/4);
				out[1][0] = row1;
			}
			static inline void odd_row(T (*out)[3], const T * previous, const T * row, const T * next){
				...
				out[0][2] = static_cast<T>((prev0 + next0)/2);
				out[0][1] = row0;
				out[0][0] = static_cast<T>((row_1+row1)/2);
				out[1][2] = static_cast<T>((prev0 + next0 + prev2 + next2)/4);
				out[1][1] = static_cast<T>((row0+row2+prev1+next1)/4);
				out[1][0] = row1;
			}
			static inline void odd_right(T (*out)[3], const T * previous, const T * row, const T * next){
				...
				out[0][1] = row0;
				out[0][0] = static_cast<T>((row_1+row1)/2);
				out[1][2] = static_cast<T>((prev0 + next0)/2);
				out[1][1] = static_cast<T>((row0+prev1+next1)/3);
				out[1][0] = row1;
			}
			...
			static inline void even_left(T (*out)[3], const T * previous, const T * row, const T * next){
				...
				out[0][2] = row0;
				out[0][1] = static_cast<T>((row1+prev0+next0)/3);
				out[0][0] = static_cast<T>((prev1+next1)/2);
				out[1][2] = static_cast<T>((row0+row2)/2);
				out[1][1] = row1;
				out[1][0] = out[0][0];
			}
			static inline void even_row(T (*out)[3], const T * previous, const T * row, const T * next){
				...
				out[0][2] = row0;
				out[0][1] = static_cast<T>((row_1+row1+prev0+next0)/4);
				out[0][0] = static_cast<T>((prev_1+next_1+prev1+next1)/4);
				out[1][2] = static_cast<T>((row0+row2)/2);
				out[1][1] = row1;
				out[1][0] = static_cast<T>((prev1 + next1)/2);
			}
			static inline void even_right(T (*out)[3], const T * previous, const T * row, const T * next){
				T prev_1 = R::get(previous[-1]);
				T prev0 = R::get(previous[0]);
			...
			static inline void lower_left(T (*out)[3], const T * previous, const T * row){
				...
				out[0][1] = row0;
				out[0][0] = row1;
				out[1][2] = static_cast<T>((prev0+prev2)/2);
				out[1][1] = static_cast<T>((row0+row2+prev1)/3);
				out[1][0] = row1;
			}
			static inline void lower_row(T (*out)[3], const T * previous, const T * row){
	...
	template <class T, class R>
		struct bayer_sample_gbrg {
			static inline void upper_left(T (*out)[3], const T * row, const T * next){
				...
				out[0][1] = row0;
				out[0][2] = row1;
				out[1][0] = static_cast<T>((next0+next2)/2);
				out[1][1] = static_cast<T>((row0+row2+next1)/3);
				out[1][2] = row1;
			}
			static inline void upper_row(T (*out)[3], const T * row, const T * next){
				T row_1 = R::get(row[-1]);
			...
			static inline void upper_right(T (*out)[3], const T * row, const T * next){
				...
				out[0][2] = static_cast<T>((row_1+row1)/2);
				out[1][0] = next0;
				out[1][1] = static_cast<T>((row0+next1)/2);
				out[1][2] = row1;
			}
			...
			static inline void lower_left(T (*out)[3], const T * previous, const T * row){
				...
				T row2 = R::get(row[2]);
				out[0][0] = row0;
				out[0][1] = static_cast<T>((prev0 + row1)/2);
				out[0][2] = prev1;
				out[1][0] = static_cast<T>((row0+row2)/2);
				out[1][1] = row1;
				out[1][2] = prev1;
			...
			static inline void lower_row(T (*out)[3], const T * previous, const T * row){
				...
				T row2 = R::get(row[2]);
				out[0][0] = row0;
				out[0][1] = static_cast<T>((row_1+prev0 + row1)/3);
				out[0][2] = static_cast<T>((prev_1+prev1)/2);
				out[1][0] = static_cast<T>((row0+row2)/2);
				out[1][1] = row1;
				out[1][2] = prev1;
			}
	...
	template <class T, class R>
		struct bayer_sample_grbg {
			static inline void upper_left(T (*out)[3], const T * row, const T * next){
				...
				out[0][1] = row0;
				out[0][0] = row1;
				out[1][2] = static_cast<T>((next0+next2)/2);
				out[1][1] = static_cast<T>((row0+row2+next1)/3);
				out[1][0] = row1;
			}
			static inline void upper_row(T (*out)[3], const T * row, const T * next){
			...
			static inline void lower_row(T (*out)[3], const T * previous, const T * row){
				...
				out[0][2] = row0;
				out[0][1] = static_cast<T>((row_1+prev0 + row1)/3);
				out[0][0] = static_cast<T>((prev_1+prev1)/2);
				out[1][2] = static_cast<T>((row0+row2)/2);
				out[1][1] = row1;
				out[1][0] = prev1;
			}
	...
	template <class T>
		inline T cie(T * c) { return static_cast<T>((c[0]*77 + c[1]*150 + c[2]*29)>>8); }
