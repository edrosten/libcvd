#ifndef GRANTA_EDGE_HPP
#define GRANTA_EDGE_HPP
...
namespace Granta {
  ...
  template <class T>
  void zero_border(SubImage <T> &im) {
    int lasty = std::max(im.size().y-1,0);
    int lastx = std::max(im.size().x-1,0);
    for (int y = 0; y < im.size().y; y++) {
      im[y][lastx] = 0;
      im[y][0] = 0;
    }
  ...
  template <class T>
  void hsobel(const SubImage <T> &im, SubImage <T> &out) {
    zero_border(out);
    for(int y=1; y < im.size().y-1; y++) {
      for(int x=1; x < im.size().x-1; x++) {
        const double xgrad = ((im[y-1][x-1] + 2 * im[y-1][x] + im[y-1][x+1]) -
                              (im[y+1][x-1] + 2 * im[y+1][x] + im[y+1][x+1]));
        out[y][x] = static_cast<T>(std::abs(xgrad) / 4.0);
      }
    }
  }

  template <class T>
  void vsobel(const SubImage <T> &im, SubImage <T> &out) {
    zero_border(out);
    for(int y=1; y < im.size().y-1; y++) {
      for(int x=1; x < im.size().x-1; x++) {
        const double ygrad = (im[y-1][x-1] + 2 * im[y][x-1] + im[y+1][x-1]) -
          (im[y-1][x+1] + 2 * im[y][x+1] + im[y+1][x+1]);
        out[y][x] = static_cast<T>(std::abs(ygrad) / 4.0);
      }
    }
  }

  template <class T>
  void gaussian_gradient_magnitude(const BasicImage <T> &im, BasicImage <T> &out, const double sigma) {
    Image <T> smoothed(out.size());
    Image <T> dirs(out.size());
    convolveGaussian(im, smoothed, sigma);
    for(int y=1; y < im.size().y-1; y++) {
      for(int x=1; x < im.size().x-1; x++) {
        T xgrad = ((smoothed[y-1][x-1] + 2 * smoothed[y-1][x] + smoothed[y-1][x+1]) -
                   (smoothed[y+1][x-1] + 2 * smoothed[y+1][x] + smoothed[y+1][x+1]));
        T ygrad = ((smoothed[y-1][x-1] + 2 * smoothed[y][x-1] + smoothed[y+1][x-1]) -
                   (smoothed[y-1][x+1] + 2 * smoothed[y][x+1] + smoothed[y+1][x+1]));
        out[y][x] = std::sqrt(xgrad * xgrad + ygrad * ygrad);
        //dirs[y][x] = std::atan(xgrad/ygrad);
        /*if (dir[y][x] != dir[y][x]) {
          dir[y][x] = 0.0;
          }*/
      }
