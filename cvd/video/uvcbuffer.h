...
namespace Exceptions
{
	...
	namespace UVCBuffer
	{
		...
		/// @ingroup gException
		struct Init : public All
		{
			Init(std::string dev, std::string err); ///< Construct from the device name
		};

		/// Error opening the device
		/// @ingroup gException
		struct DeviceOpen : public All
		{
			DeviceOpen(std::string dev, std::string err); ///< Construct from the device name
		};

		/// Error opening the device
		/// @ingroup gException
		struct StreamOpen : public All
		{
			StreamOpen(std::string dev, std::string err); ///< Construct from the device name
		};

		/// Error opening the device
		/// @ingroup gException
		struct StreamStart : public All
		{
			StreamStart(std::string dev, std::string err); ///< Construct from the device name
		};

		/// Error opening the device
		/// @ingroup gException
		struct FindDevice : public All
		{
			FindDevice(std::string dev, std::string err); ///< Construct from the device name
		};

		/// Error setting up the device
		/// @ingroup gException
		struct DeviceSetup : public All
		{
			DeviceSetup(std::string dev, std::string action); ///< Construct from the device string and an error string
		};
		/// Error in a put_frame() call
		/// @ingroup gException
		struct PutFrame : public All
		{
			PutFrame(std::string dev, std::string msg); ///< Construct from the device name
		};
		/// Error in a get_frame() call
		/// @ingroup gException
		struct GetFrame : public All
		{
			GetFrame(std::string dev, std::string msg); ///< Construct from the device name
		};
	}
}
...
template <class T>
class UVCBuffer : public VideoBuffer<T>, public UVC::RawUVCBuffer
{
	public:
	UVCBuffer(const std::string& dev, ImageRef size, int frames_per_second = 0, bool mjpeg = 0, bool verbose = 0)
	    : VideoBuffer<T>(VideoBufferType::Live)
	    , RawUVCBuffer(dev, UVC::format<T>::fmt, size, frames_per_second, mjpeg, verbose)
	{
	}
